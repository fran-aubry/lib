...
public class Arrays2 {
	...
	public static boolean alleq(int[] a) {
	...
	public static HashMap<Integer, LinkedList<Integer>> toIndexMap(int[] a) {
		HashMap<Integer, LinkedList<Integer>> M = new HashMap<>();
		for(int i = 0; i < a.length; i++) {
			int x = a[i];
			LinkedList<Integer> xset = M.get(x);
			if(xset == null) {
				xset = new LinkedList<Integer>();
				M.put(x, xset);
			}
			xset.add(i);
		}
	...
	public static HashMap<Integer, Integer> toCountMap(int[] a) {
	...
	public static String matrixToStr(int[][] m, String colSep, String rowSep) {
		StringBuilder sb = new StringBuilder();
		for(int i = 0; i < m.length; i++) {
			for(int j = 0; j < m[i].length; j++) {
				sb.append(m[i][j]);
	...
	public static String matrixToStr(int[][] m) {
		return matrixToStr(m, "\t", "\n");
	}
	...
	public static void fill(int[][] m, int val) {
		for(int i = 0; i < m.length; i++) {
			Arrays.fill(m[i], val);
		}
	}
