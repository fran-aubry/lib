...
public class RandomExt {
	...
	public static EdgeFunction genUndEdgeFunction(Graph g, int minw, int maxw) {
		EdgeFunction f = new EdgeFunction();
		for(Edge e : g.edges()) {
			if(!f.defined(e) && !f.defined(e.reverse())) {
				int w = genInt(minw, maxw);
				f.set(e, w);
				f.set(e.reverse(), w);
			}
		}
